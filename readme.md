...
- **히어로 마스크 리빌 애니메이션** — SplitChars로 분할한 문자별 y-position 마스크 애니메이션, '소음에서 울림으로 / 경험을 조율하는 / 안건 입니다' 3행 순차 등장→대기→퇴장 무한반복, 뷰포트 이탈 시 자동 일시정지
- **히어로 패럴렉스** — 스크롤 시 프로필 사진 시차 이동으로 깊이감 연출
...
- **프로젝트 카드 글래스모피즘** — 각 프로젝트 카드에 backdrop-filter 반투명 블러 배경, 보더, 박스쉐도우 적용
...
- **파비콘** — 커스텀 SVG 파비콘 (화이트)
- **빌드 최적화** — Vite manualChunks로 vendor(React 계열)·animation(GSAP·Lenis) 코드 청크 분리
...
│   │   ├── Hero.jsx           # 히어로 배너, 마스크 리빌 텍스트 애니메이션, 패럴렉스
│   │   ├── About.jsx          # 자기소개 섹션 (인적사항, 학력 및 교육사항)
│   │   ├── Projects.jsx       # 프로젝트 1~5, 핀 스크롤, 글래스 카드
...
│   │   └── CustomCursor.jsx   # 8분음표 커스텀 커서 + 클릭 ripple (현재 미사용)
│   ├── lib/
...
- [x] 접근성(a11y) 보완 (완료)
- [x] 히어로 텍스트 마스크 리빌 애니메이션 (완료)
- [x] 프로젝트 카드 글래스모피즘 디자인 (완료)
- [ ] 모바일 반응형 레이아웃 세부 조정
- [ ] 프로젝트 상세 페이지 콘텐츠 작성
